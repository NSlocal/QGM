...
if ! "$cygwin" && ! "$darwin" && ! "$nonstop" ; then
    case $MAX_FD in
      max*) MAX_FD=$( ulimit -H -n ) || warn "Could not query fd limit" ;;
    esac
    case $MAX_FD in
      '' | soft) ;;
      *) ulimit -n "$MAX_FD" || warn "Could not set fd limit to $MAX_FD" ;;
    esac
fi
...
xargs -n1 < "$JVM_OPTS_FILE" 2>/dev/null | while IFS= read -r arg; do
    [ -n "$arg" ] && set -- "$arg" "$@"
done
...
if [ "$(uname)" = "Darwin" ] && [ -z "$JAVA_HOME" ] ; then cd "$(dirname "$0")"; fi
